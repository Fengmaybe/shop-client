<template>
  <div class="shop-info" ref="ScrollY_Ul">
    <div class="info-content">
      ...
      <section class="section">
        ...
        <div class="pic-wrapper" ref="ScrollX_Ul">
          <ul class="pic-list" ref="pics_ul">
...
<script>
  ...
  export default {
    ...
    mounted () {
      //当从别的路由切换过来的时候数据已经有了，mounted就是界面初始化已经完成了
      if(this.info.pics){
        this._initScroll();
      }
    ...
    methods:{
      _initScroll () {
        new BScroll(this.$refs.ScrollY_Ul);

        //获取ul的宽度
        const li = 120;
        const space = 6;
        const countLi = this.info.pics.length;

        const ulWidth = (li + space) * countLi - space;
        this.$refs.pics_ul.style.width = ulWidth+'px';
        new BScroll(this.$refs.ScrollX_Ul,{
          scrollX:true,
        });
      }
    }
